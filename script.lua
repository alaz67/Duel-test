-- Secret Duel Hub - Animation Changer & Hub
-- Black & White Theme with Texture ID 6974075369
...
-- SECRET DUEL ANIMATIONS
local SecretDuelAnimations = {
    ...
    Adidas = {["idle.Animation1"]="18537376492",["idle.Animation2"]="18537371272",["walk.WalkAnim"]="18537392113",["run.RunAnim"]="18537384940",["jump.JumpAnim"]="18537380791",["climb.ClimbAnim"]="18537363391",["fall.FallAnim"]="18537367238",["swim.Swim"]="18537389531",["swimidle.SwimIdle"]="18537387180",["pose.StylishPose"]="18537374150"},
...
-- SECRET DUEL GUI REFERENCE
local secretDuelGUI = nil
local secretDuelEnabled = false
local secretDuelVisible = false
local setSecretDuelVisual = nil
...
local V = {
	...
	themeAccent=nil,
	sidebarArt="6974075369",
	secretDuelEnabled=false,
	mobileMode=false,
	pcMode=true,
}
...
local PlayerESP = {enabled = false, playerData = {}, conns = {}, discordText = "discord.gg/secrethub"}
...
local CURRENT_ART_ID = "6974075369"
...
do
	...
	local function _espSetupCharacter(player, char)
		...
		hl.Name = "SecretESP"; hl.Adornee = char
		...
		bb.Name = "SecretESPTag"; bb.Adornee = head
...
local function _v4mpClearSky()
    for _, v in ipairs(Lighting:GetChildren()) do
        if v:GetAttribute("_SecretSky") then pcall(function() v:Destroy() end) end
    end
...
local function applyCustomSky(mode)
    ...
    if preset.sky then
        ...
        sky:SetAttribute("_SecretSky", true)
    ...
    if preset.atm then
        ...
        atm:SetAttribute("_SecretSky", true)
    ...
    if preset.clouds and terrain then
        ...
        clouds:SetAttribute("_SecretSky", true)
...
local function enablePotatoGraphics()
    ...
    pcall(function()
        ...
        local sky = Instance.new("Sky"); sky.Name = "_SecretPotato"
...
local function disablePotatoGraphics()
    ...
    pcall(function()
        local s = Lighting:FindFirstChild("_SecretPotato")
...
local function saveConfig()
	...
	local cfg={
		...
		secretDuelEnabled=secretDuelEnabled,
...
-- ============================================================
-- SECRET ANIM CHANGER GUI (Animation Window - separate from main hub)
-- ============================================================
local function buildSecretAnimChanger()
	local Animate = LP.Character and LP.Character:FindFirstChild("Animate")
	
	local old = CoreGui:FindFirstChild("SecretAnimChanger")
	if old then old:Destroy() end
	
	local guiAC = Instance.new("ScreenGui")
	guiAC.Name = "SecretAnimChanger"
	guiAC.ResetOnSpawn = false
	guiAC.Parent = CoreGui
	
	-- Black & White Colors
	local COL_BG = Color3.fromRGB(0, 0, 0)
	local COL_ROW = Color3.fromRGB(20, 20, 20)
	local COL_HOVER = Color3.fromRGB(40, 40, 40)
	local COL_WHITE = Color3.fromRGB(255, 255, 255)
	local COL_BORDER = Color3.fromRGB(60, 60, 60)
	
	local mainAC = Instance.new("Frame", guiAC)
	mainAC.Size = UDim2.new(0, 340, 0, 440)
	mainAC.Position = UDim2.new(0.5, -170, 0.5, -220)
	mainAC.BackgroundColor3 = COL_BG
	mainAC.BackgroundTransparency = 0
	mainAC.BorderSizePixel = 0
	mainAC.ClipsDescendants = true
	mainAC.Active = true
	mainAC.Draggable = true
	mainAC.Visible = secretDuelVisible
	Instance.new("UICorner", mainAC).CornerRadius = UDim.new(0, 12)
	
	-- Background Texture ID 6974075369
	local bgImage = Instance.new("ImageLabel", mainAC)
	bgImage.Size = UDim2.new(1, 0, 1, 0)
	bgImage.Position = UDim2.new(0, 0, 0, 0)
	bgImage.BackgroundTransparency = 1
	bgImage.Image = "rbxassetid://6974075369"
	bgImage.ScaleType = Enum.ScaleType.Crop
	bgImage.ImageTransparency = 0.7
	bgImage.ZIndex = 1
	
	local bgOverlay = Instance.new("Frame", mainAC)
	...
	local borderStroke = Instance.new("UIStroke", mainAC)
	borderStroke.Color = COL_BORDER
	borderStroke.Thickness = 1.5
	borderStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
	
	local titleAC = Instance.new("TextLabel", mainAC)
	titleAC.Size = UDim2.new(1, -20, 0, 24)
	titleAC.Position = UDim2.new(0, 10, 0, 6)
	titleAC.BackgroundTransparency = 1
	titleAC.Text = "SECRET ANIM CHANGER"
	titleAC.Font = Enum.Font.GothamBlack
	titleAC.TextSize = 15
	titleAC.TextColor3 = COL_WHITE
	titleAC.TextXAlignment = Enum.TextXAlignment.Left
	titleAC.ZIndex = 3
	
	local subtitleAC = Instance.new("TextLabel", mainAC)
	subtitleAC.Size = UDim2.new(1, -20, 0, 14)
	subtitleAC.Position = UDim2.new(0, 10, 0, 30)
	subtitleAC.BackgroundTransparency = 1
	subtitleAC.Text = "discord.gg/secrethub"
	subtitleAC.Font = Enum.Font.Gotham
	subtitleAC.TextSize = 10
	subtitleAC.TextColor3 = Color3.fromRGB(160, 160, 160)
	subtitleAC.TextXAlignment = Enum.TextXAlignment.Left
	subtitleAC.ZIndex = 3
	
	local minBtnAC = Instance.new("TextButton", mainAC)
	minBtnAC.Size = UDim2.new(0, 26, 0, 26)
	minBtnAC.Position = UDim2.new(1, -34, 0, 6)
	minBtnAC.BackgroundColor3 = COL_ROW
	minBtnAC.Text = "◄"
	minBtnAC.TextColor3 = COL_WHITE
	minBtnAC.Font = Enum.Font.GothamBold
	minBtnAC.TextSize = 12
	minBtnAC.BorderSizePixel = 0
	minBtnAC.AutoButtonColor = false
	minBtnAC.ZIndex = 3
	Instance.new("UICorner", minBtnAC).CornerRadius = UDim.new(0, 6)
	
	local minimizedAC = false
	minBtnAC.MouseButton1Click:Connect(function()
		minimizedAC = not minimizedAC
		TS:Create(mainAC, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {
			Size = minimizedAC and UDim2.new(0, 340, 0, 40) or UDim2.new(0, 340, 0, 440)
		}):Play()
		minBtnAC.Text = minimizedAC and "►" or "◄"
		contentAC.Visible = not minimizedAC
		subtitleAC.Visible = not minimizedAC
	end)
	
	local contentAC = Instance.new("Frame", mainAC)
	contentAC.Size = UDim2.new(1, -16, 1, -56)
	contentAC.Position = UDim2.new(0, 8, 0, 48)
	contentAC.BackgroundTransparency = 1
	contentAC.ClipsDescendants = true
	contentAC.ZIndex = 3
	
	local scrollAC = Instance.new("ScrollingFrame", contentAC)
	scrollAC.Size = UDim2.new(1, 0, 1, 0)
	scrollAC.BackgroundTransparency = 1
	scrollAC.BorderSizePixel = 0
	scrollAC.CanvasSize = UDim2.new(0, 0, 0, 0)
	scrollAC.ScrollBarThickness = 3
	scrollAC.ScrollBarImageTransparency = 0.4
	scrollAC.ClipsDescendants = true
	scrollAC.ZIndex = 3
	
	local scrollLayoutAC = Instance.new("UIListLayout", scrollAC)
	scrollLayoutAC.SortOrder = Enum.SortOrder.LayoutOrder
	scrollLayoutAC.Padding = UDim.new(0, 4)
	
	local scrollPaddingAC = Instance.new("UIPadding", scrollAC)
	scrollPaddingAC.PaddingLeft = UDim.new(0, 4)
	scrollPaddingAC.PaddingRight = UDim.new(0, 4)
	scrollPaddingAC.PaddingTop = UDim.new(0, 4)
	scrollPaddingAC.PaddingBottom = UDim.new(0, 4)
	
	local function createCategoryHeaderAC(text)
		local frame = Instance.new("Frame", scrollAC)
		...
		line.BackgroundColor3 = COL_BORDER
	...
	local function createAnimationButtonAC(buttonText, animData)
		local btn = Instance.new("TextButton", scrollAC)
		btn.Size = UDim2.new(1, 0, 0, 32)
		btn.BackgroundColor3 = COL_ROW
		...
		statusIcon.Position = UDim2.new(1, -28, 0, 0)
		statusIcon.BackgroundTransparency = 1
		statusIcon.Text = ""
		statusIcon.Font = Enum.Font.GothamBold
		statusIcon.TextSize = 14
		statusIcon.TextColor3 = COL_WHITE
		statusIcon.TextXAlignment = Enum.TextXAlignment.Right
		statusIcon.ZIndex = 4
		...
		btn.MouseEnter:Connect(function()
			if not busy then
				TS:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = COL_HOVER}):Play()
			end
		end)

		btn.MouseLeave:Connect(function()
			if not busy then
				TS:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = COL_ROW}):Play()
			end
		end)

		btn.MouseButton1Click:Connect(function()
			if busy or not Animate then return end
			busy = true
			
			statusIcon.Text = "✓"
			TS:Create(btn, TweenInfo.new(0.15), {BackgroundColor3 = COL_WHITE}):Play()
	...
	createCategoryHeaderAC("NORMAL")
	local sortedNormal = {}
	for name in pairs(SecretDuelAnimations) do
		if name ~= "None" and name ~= "Anthro" and 
		   name ~= "Patrol" and name ~= "Confident" and name ~= "Popstar" and 
		   name ~= "Sneaky" and name ~= "Princess" and name ~= "Cowboy" and name ~= "Ghost" then
			table.insert(sortedNormal, name)
		end
	end
	table.sort(sortedNormal)
	for _, name in ipairs(sortedNormal) do
		createAnimationButtonAC(name, SecretDuelAnimations[name])
	end
	
	createCategoryHeaderAC("SPECIAL")
	local sortedSpecial = {"Patrol","Confident","Popstar","Sneaky","Princess","Cowboy","Ghost"}
	for _, name in ipairs(sortedSpecial) do
		createAnimationButtonAC(name, SecretDuelAnimations[name])
	end
	
	createCategoryHeaderAC("OTHER")
	createAnimationButtonAC("None", SecretDuelAnimations["None"])
	createAnimationButtonAC("Anthro", SecretDuelAnimations["Anthro"])
	
	-- Keybind: RightShift to toggle
	UIS.InputBegan:Connect(function(input, gpe)
		if gpe then return end
		if input.KeyCode == Enum.KeyCode.RightShift then
			guiAC.Enabled = not guiAC.Enabled
		end
	end)
...
local function buildGui()
	...
	local old=game:GetService("CoreGui"):FindFirstChild("SecretDuelHub");if old then old:Destroy() end
	local pg=LP:FindFirstChild("PlayerGui");if pg then local o=pg:FindFirstChild("SecretDuelHub");if o then o:Destroy() end end
	local gui=Instance.new("ScreenGui")
	gui.Name="SecretDuelHub";gui.ResetOnSpawn=false;gui.DisplayOrder=10;gui.IgnoreGuiInset=true
	...
	-- Background Texture ID 6974075369 on main panel
	local mainBg = Instance.new("ImageLabel", main)
	mainBg.Size = UDim2.new(1, 0, 1, 0)
	mainBg.Position = UDim2.new(0, 0, 0, 0)
	mainBg.BackgroundTransparency = 1
	mainBg.Image = "rbxassetid://6974075369"
	mainBg.ScaleType = Enum.ScaleType.Crop
	mainBg.ImageTransparency = 0.85
	mainBg.ZIndex = 0
	
	local panelScrim = Instance.new("Frame", main)
	panelScrim.Name = "PanelScrim";panelScrim.Size=UDim2.new(1,0,1,0);panelScrim.BackgroundColor3=Color3.fromRGB(0,0,0);panelScrim.BackgroundTransparency=0.6;panelScrim.ZIndex=1
	...
	-- Sidebar Image (Texture ID 6974075369)
	...
	sidebarImg.Image = "rbxassetid://6974075369"
	...
	tabContainer.Size = UDim2.new(0, 122, 0, 226)
	tabContainer.AnchorPoint = Vector2.new(0.5, 0.5)
	tabContainer.Position = UDim2.new(0.50, 0, 0.54, 0)
	...
	miniBtn.Text="Secret Hub";miniBtn.TextColor3=THEME_ACCENT;miniBtn.Font=Enum.Font.GothamBold;miniBtn.TextSize=12
	...
	contentArea.Position = UDim2.new(0, (SIDEBAR_W - CONTENT_OVERLAP) + MENU_GAP, 0, 0)
	...
	splitCover.BackgroundTransparency = 0.2
	...
	local SECTION_TO_PAGE = {
		...
		Movement = otherPage, Teleport = otherPage, ["Auto Left / Right"] = otherPage,
		Steal = configPage, Visual = configPage, Display = configPage, Interface = configPage, Config = configPage,
		["Grab Radius"] = configPage, ["Steal Duration"] = configPage, FOV = configPage, ["Mode Select"] = configPage,
	}
	...
	local function mkRow(h)
		local f=Instance.new("Frame",sf);f.Size=UDim2.new(1,-2,0,h or 40)
		f.BackgroundColor3=Color3.fromRGB(0, 0, 0);f.BackgroundTransparency=0.3;f.BorderSizePixel=0;f.LayoutOrder=LO()
		f.ZIndex=6
		Instance.new("UICorner",f).CornerRadius=UDim.new(0,10)
		local rs=Instance.new("UIStroke",f); rs.Color=Color3.fromRGB(75, 75, 75); rs.Thickness=1; rs.Transparency=0.22
		f.MouseEnter:Connect(function() TS:Create(f,TweenInfo.new(0.08),{BackgroundColor3=Color3.fromRGB(12, 12, 12),BackgroundTransparency=0.5}):Play() end)
		f.MouseLeave:Connect(function() TS:Create(f,TweenInfo.new(0.08),{BackgroundColor3=Color3.fromRGB(0, 0, 0),BackgroundTransparency=0.3}):Play() end)
		return f
	end
	...
	local function mkKB(parent,kbEntry,cb)
		...
		local kbSt=Instance.new("UIStroke",btn);kbSt.Color=Color3.fromRGB(80, 80, 80);kbSt.Thickness=1;kbSt.Transparency=0.28
	...
	local function mkToggleKB(txt,kbEntry,onToggle,onKB)
		...
		local pill,dot=mkPill(row,kbEntry and 134 or 56)
		local on=false
		local function sv(s) on=s;animPill(pill,dot,s) end
		local clk=Instance.new("TextButton",pill);clk.Size=UDim2.new(1,0,1,0);clk.BackgroundTransparency=1;clk.Text="";clk.ZIndex=5
	...
	-- SECRET ANIM CHANGER TOGGLE IN INTERFACE TAB
	local sdRow = mkRow(40)
	mkLabel(sdRow, "Secret Anim Changer")
	local sdPill, sdDot = mkPill(sdRow, 56)
	local sdOn = secretDuelEnabled
	local function setSDVisual(s)
		sdOn = s
		animPill(sdPill, sdDot, s)
		secretDuelEnabled = s
		V.secretDuelEnabled = s
		if s then
			if not secretDuelGUI then buildSecretAnimChanger() end
			if secretDuelGUI then
				secretDuelVisible = true
				secretDuelGUI.Visible = true
				secretDuelGUI.Enabled = true
			end
		else
			if secretDuelGUI then
				secretDuelVisible = false
				secretDuelGUI.Visible = false
				secretDuelGUI.Enabled = false
			end
		end
		saveConfig()
	end
	setSDVisual(sdOn)
	setSecretDuelVisual = setSDVisual
	
	local sdClk = Instance.new("TextButton", sdPill)
	sdClk.Size = UDim2.new(1,0,1,0)
	sdClk.BackgroundTransparency = 1
	sdClk.Text = ""
	sdClk.ZIndex = 5
	sdClk.Activated:Connect(function()
		if _anyKeyListening then return end
		setSDVisual(not sdOn)
	end)
	
	-- MODE SELECT (Mobile/PC)
	mkSectMerge("Mode Select")
	do
		local row = mkRow(40)
		mkLabel(row, "Platform Mode")
		
		-- Mobile Button
		local mobileBtn = Instance.new("TextButton", row)
		mobileBtn.Size = UDim2.new(0, 70, 0, 26)
		mobileBtn.Position = UDim2.new(1, -150, 0.5, -13)
		mobileBtn.BackgroundColor3 = V.mobileMode and THEME_ACCENT or Color3.fromRGB(42, 42, 42)
		mobileBtn.BorderSizePixel = 0
		mobileBtn.Text = "MOBILE"
		mobileBtn.TextColor3 = V.mobileMode and Color3.fromRGB(0,0,0) or Color3.fromRGB(255,255,255)
		...
		-- PC Button
		local pcBtn = Instance.new("TextButton", row)
		pcBtn.Size = UDim2.new(0, 70, 0, 26)
		pcBtn.Position = UDim2.new(1, -74, 0.5, -13)
		pcBtn.BackgroundColor3 = V.pcMode and THEME_ACCENT or Color3.fromRGB(42, 42, 42)
		pcBtn.BorderSizePixel = 0
		pcBtn.Text = "PC"
		pcBtn.TextColor3 = V.pcMode and Color3.fromRGB(0,0,0) or Color3.fromRGB(255,255,255)
		...
		local function updateModeUI()
			...
			-- Show/hide mobile buttons based on mode
			for _, mb in pairs(mobileButtons) do
				if mb and mb.btn then
					mb.btn.Visible = V.mobileMode
				end
			end
			
			-- Scale main GUI for mobile (2x smaller)
			if V.mobileMode then
	...
	-- MOBILE BUTTONS (only visible in mobile mode)
	local mobileButtons = {}
	local function makeMobileButton(key, label, onPress)
		...
		local function setActive(state)
			...
			TS:Create(btn, TweenInfo.new(0.18), {
				BackgroundColor3 = state and THEME_ACCENT or Color3.fromRGB(10, 10, 10),
				TextColor3 = state and Color3.fromRGB(255,255,255) or Color3.fromRGB(225, 225, 225),
			}):Play()
			TS:Create(stroke, TweenInfo.new(0.18), {
				Color = state and Color3.fromRGB(235, 235, 235) or Color3.fromRGB(70, 70, 70),
	...
	makeMobileButton("laggerNormal", "LAG\nNORM", function()
	...
	makeMobileButton("laggerCarry", "LAG\nCARRY", function()
	...
	if _introEnabled then
		...
		task.spawn(function()
			...
			title.BackgroundTransparency = 1; title.Text = "Secret Hub"
			...
			subtitle.BackgroundTransparency = 1; subtitle.Text = "discord.gg/secrethub"
			...
			TS:Create(darkBg, TweenInfo.new(0.5), {BackgroundTransparency = 0.5}):Play()
...
local function loadConfigKeys()
	if not(isfile and isfile("SecretDuel.json")) then return end
	local ok,cfg=pcall(function() return HS:JSONDecode(readfile("SecretDuel.json")) end)
	...
	if cfg.secretDuelEnabled ~= nil then secretDuelEnabled = cfg.secretDuelEnabled end
	if cfg.mobileMode ~= nil then V.mobileMode = cfg.mobileMode end
	if cfg.pcMode ~= nil then V.pcMode = cfg.pcMode end
end
...
local function loadConfigState()
	...
	-- Build Secret Anim Changer if enabled
	if secretDuelEnabled then
		task.spawn(function()
			task.wait(0.5)
			if not secretDuelGUI then buildSecretAnimChanger() end
			if secretDuelGUI then
				...
				secretDuelGUI.Visible = true
				secretDuelGUI.Enabled = true
				if setSecretDuelVisual then setSecretDuelVisual(true) end
			end
		end)
	end
...
buildGui()
loadConfigState()
print("Secret Duel Hub Loaded")
print("discord.gg/secrethub")
