-- Secret Duel - Animation Changer (Black & White Theme with Texture ID 6974075369)
-- Based on V4MP/HARMLESS HUB by Vezy/Vynx
...
-- GLOBALS FOR MOBILE BUTTONS HIDE/SIZE
_G._VezyHideSideBtns = _G._VezyHideSideBtns or false
_G._VynxMobileBtnScale = _G._VynxMobileBtnScale or 1.0

-- ===== SECRET DUEL ANIMATION DATA =====
...
-- ===== SECRET DUEL GUI =====
local function buildSecretDuelGUI()
        local Animate = LP.Character and LP.Character:FindFirstChild("Animate")
        
        -- Remove old GUI
        local old = CoreGui:FindFirstChild("SecretDuel")
        if old then old:Destroy() end
        
        local guiSD = Instance.new("ScreenGui")
        guiSD.Name = "SecretDuel"
        guiSD.ResetOnSpawn = false
        guiSD.Parent = CoreGui
        ...
        local COL_DARK = Color3.fromRGB(10, 10, 10)
        
        -- Main Frame
        local mainSD = Instance.new("Frame", guiSD)
        mainSD.Size = UDim2.new(0, 320, 0, 420)
        mainSD.Position = UDim2.new(0.5, -160, 0.5, -210)
        mainSD.BackgroundColor3 = COL_BG
        mainSD.BackgroundTransparency = 0
        mainSD.BorderSizePixel = 0
        mainSD.ClipsDescendants = true
        mainSD.Active = true
        mainSD.Draggable = true
        Instance.new("UICorner", mainSD).CornerRadius = UDim.new(0, 12)
        
        -- Background Texture (ID 6974075369)
        local bgImage = Instance.new("ImageLabel", mainSD)
        ...
        local bgOverlay = Instance.new("Frame", mainSD)
        ...
        -- Border
        local borderStroke = Instance.new("UIStroke", mainSD)
        borderStroke.Color = COL_BORDER
        borderStroke.Thickness = 1.5
        borderStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        
        -- Title
        local titleSD = Instance.new("TextLabel", mainSD)
        titleSD.Size = UDim2.new(1, -20, 0, 24)
        titleSD.Position = UDim2.new(0, 10, 0, 6)
        titleSD.BackgroundTransparency = 1
        titleSD.Text = "SECRET DUEL"
        titleSD.Font = Enum.Font.GothamBlack
        titleSD.TextSize = 18
        titleSD.TextColor3 = COL_WHITE
        titleSD.TextXAlignment = Enum.TextXAlignment.Left
        titleSD.ZIndex = 3
        
        local subtitleSD = Instance.new("TextLabel", mainSD)
        subtitleSD.Size = UDim2.new(1, -20, 0, 14)
        subtitleSD.Position = UDim2.new(0, 10, 0, 30)
        subtitleSD.BackgroundTransparency = 1
        subtitleSD.Text = ".gg/secrethub"
        subtitleSD.Font = Enum.Font.Gotham
        subtitleSD.TextSize = 10
        subtitleSD.TextColor3 = Color3.fromRGB(160, 160, 160)
        subtitleSD.TextXAlignment = Enum.TextXAlignment.Left
        subtitleSD.ZIndex = 3
        
        -- Minimize Button
        local minBtnSD = Instance.new("TextButton", mainSD)
        minBtnSD.Size = UDim2.new(0, 26, 0, 26)
        minBtnSD.Position = UDim2.new(1, -34, 0, 6)
        minBtnSD.BackgroundColor3 = COL_DARK
        minBtnSD.Text = "◄"
        minBtnSD.TextColor3 = COL_WHITE
        minBtnSD.Font = Enum.Font.GothamBold
        minBtnSD.TextSize = 12
        minBtnSD.BorderSizePixel = 0
        minBtnSD.AutoButtonColor = false
        minBtnSD.ZIndex = 3
        Instance.new("UICorner", minBtnSD).CornerRadius = UDim.new(0, 6)
        
        local minimizedSD = false
        minBtnSD.MouseButton1Click:Connect(function()
            minimizedSD = not minimizedSD
            TweenService:Create(mainSD, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {
                Size = minimizedSD and UDim2.new(0, 320, 0, 40) or UDim2.new(0, 320, 0, 420)
            }):Play()
            minBtnSD.Text = minimizedSD and "►" or "◄"
            contentSD.Visible = not minimizedSD
            subtitleSD.Visible = not minimizedSD
        end)
        
        -- Content Frame
        local contentSD = Instance.new("Frame", mainSD)
        contentSD.Size = UDim2.new(1, -16, 1, -56)
        contentSD.Position = UDim2.new(0, 8, 0, 48)
        contentSD.BackgroundTransparency = 1
        contentSD.ClipsDescendants = true
        contentSD.ZIndex = 3
        
        -- Scrolling Frame
        local scrollSD = Instance.new("ScrollingFrame", contentSD)
        scrollSD.Size = UDim2.new(1, 0, 1, 0)
        scrollSD.BackgroundTransparency = 1
        scrollSD.BorderSizePixel = 0
        scrollSD.CanvasSize = UDim2.new(0, 0, 0, 0)
        scrollSD.ScrollBarThickness = 3
        scrollSD.ScrollBarImageTransparency = 0.4
        scrollSD.ClipsDescendants = true
        scrollSD.ZIndex = 3
        
        local scrollLayoutSD = Instance.new("UIListLayout", scrollSD)
        scrollLayoutSD.SortOrder = Enum.SortOrder.LayoutOrder
        scrollLayoutSD.Padding = UDim.new(0, 4)
        
        local scrollPaddingSD = Instance.new("UIPadding", scrollSD)
        scrollPaddingSD.PaddingLeft = UDim.new(0, 4)
        scrollPaddingSD.PaddingRight = UDim.new(0, 4)
        scrollPaddingSD.PaddingTop = UDim.new(0, 4)
        scrollPaddingSD.PaddingBottom = UDim.new(0, 4)
        
        -- Category Header
        local function createCategoryHeaderSD(text)
            local frame = Instance.new("Frame", scrollSD)
        ...
        -- Animation Button
        local function createAnimationButtonSD(buttonText, animData)
            local btn = Instance.new("TextButton", scrollSD)
            ...
            btn.MouseEnter:Connect(function()
                if not busy then
                    TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = COL_HOVER}):Play()
                end
            end)

            btn.MouseLeave:Connect(function()
                if not busy then
                    TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = COL_ROW}):Play()
                end
            end)
            ...
            btn.MouseButton1Click:Connect(function()
                ...
                statusIcon.Text = "✓"
                TweenService:Create(btn, TweenInfo.new(0.15), {BackgroundColor3 = COL_WHITE}):Play()
                TweenService:Create(label, TweenInfo.new(0.15), {TextColor3 = Color3.fromRGB(0, 0, 0)}):Play()
                ...
                task.delay(0.4, function()
                    statusIcon.Text = ""
                    TweenService:Create(btn, TweenInfo.new(0.15), {BackgroundColor3 = COL_ROW}):Play()
                    TweenService:Create(label, TweenInfo.new(0.15), {TextColor3 = COL_WHITE}):Play()
                    busy = false
                end)
            end)
        ...
        for _, name in ipairs(sortedNormal) do
            createAnimationButtonSD(name, SecretDuelAnimations[name])
        end
        
        -- Special Category
        createCategoryHeaderSD("SPECIAL")
        ...
        -- Other Category
        createCategoryHeaderSD("OTHER")
        createAnimationButtonSD("None", SecretDuelAnimations["None"])
        createAnimationButtonSD("Anthro", SecretDuelAnimations["Anthro"])
        
        -- Keybind
        UIS.InputBegan:Connect(function(input, gpe)
            if gpe then return end
            if input.KeyCode == Enum.KeyCode.RightShift then
                guiSD.Enabled = not guiSD.Enabled
            end
        end)
        
        -- Update Animate on respawn
        LP.CharacterAdded:Connect(function(char)
            task.wait(0.5)
...
local function buildGui()
        ...
        local old=game:GetService("CoreGui"):FindFirstChild("V4MP");if old then old:Destroy() end
        local pg=LP:FindFirstChild("PlayerGui");if pg then local o=pg:FindFirstChild("V4MP");if o then o:Destroy() end end
        local gui=Instance.new("ScreenGui")
        gui.Name="V4MP";gui.ResetOnSpawn=false;gui.DisplayOrder=10;gui.IgnoreGuiInset=true
        ...
        main.Size=UDim2.new(0,420,0,490);main.Position=UDim2.new(1,-440,0,20)
        main.BackgroundColor3=Color3.fromRGB(6,6,6);main.BorderSizePixel=0;main.ClipsDescendants=true
        Instance.new("UICorner",main).CornerRadius=UDim.new(0,12)
        local panelBg = Instance.new("ImageLabel", main)
        panelBg.Name = "PanelBg"
        panelBg.Size = UDim2.new(1, 0, 1, 0)
        panelBg.Position = UDim2.fromOffset(0, 0)
        panelBg.BackgroundTransparency = 1
        panelBg.Image = "rbxassetid://117085976067902"
        panelBg.ScaleType = Enum.ScaleType.Stretch
        panelBg.ImageTransparency = 0
        panelBg.ZIndex = 1
        ...
        panelScrim.BackgroundTransparency = 0.7
        ...
        hdr.Size=UDim2.new(1,0,0,46);hdr.BackgroundColor3=BG2;hdr.BackgroundTransparency=0.2;hdr.BorderSizePixel=0
        hdr.ZIndex=3
        ...
        ttl.Size=UDim2.new(0,120,0,22);ttl.Position=UDim2.new(0,46,0,4)
        ttl.BackgroundTransparency=1;ttl.Text="SECRET DUEL"
        ttl.TextColor3=RED;ttl.Font=Enum.Font.GothamBlack;ttl.TextSize=14
        ttl.TextXAlignment=Enum.TextXAlignment.Left
        ttl.ZIndex=4
        local discordLbl=Instance.new("TextLabel",hdr)
        discordLbl.Size=UDim2.new(0,110,0,13);discordLbl.Position=UDim2.new(0,46,0,27)
        discordLbl.BackgroundTransparency=1;discordLbl.Text=".gg/secrethub"
        discordLbl.TextColor3=Color3.fromRGB(130,130,130)
        discordLbl.Font=Enum.Font.GothamBold;discordLbl.TextSize=9
        discordLbl.TextXAlignment=Enum.TextXAlignment.Left
        discordLbl.ZIndex=4
        local statsLbl=Instance.new("TextLabel",hdr)
        statsLbl.Size=UDim2.new(1,-176,1,0);statsLbl.Position=UDim2.new(0,130,0,0)
        statsLbl.BackgroundTransparency=1
        statsLbl.Text="Ping: -- · FPS: --"
        statsLbl.TextColor3=Color3.fromRGB(180,180,180)
        statsLbl.Font=Enum.Font.GothamBold;statsLbl.TextSize=11
        ...
        closeBtn.Text="-";closeBtn.TextColor3=REDDIM;closeBtn.Font=Enum.Font.GothamBold;closeBtn.TextSize=23
        closeBtn.ZIndex=4
        ...
        miniBtn.Text="SECRET DUEL";miniBtn.TextColor3=RED;miniBtn.Font=Enum.Font.GothamBold;miniBtn.TextSize=12
        ...
        contentSt.Color = STROKE; contentSt.Thickness = 1; contentSt.Transparency = 0.5
        ...
        tabBarSt.Color = STROKE; tabBarSt.Thickness = 1; tabBarSt.Transparency = 0.5
        ...
        local function mkSect(txt)
                ...
                local f=Instance.new("Frame",page);f.Size=UDim2.new(1,0,0,2);f.BackgroundTransparency=1;f.BorderSizePixel=0;f.LayoutOrder=LO()
        ...
        local function mkSectMerge(txt)
                local sp=Instance.new("Frame",sf);sp.Size=UDim2.new(1,0,0,4);sp.BackgroundTransparency=1;sp.BorderSizePixel=0;sp.LayoutOrder=LO()
        ...
        local function mkRow(h)
                local f=Instance.new("Frame",sf);f.Size=UDim2.new(1,0,0,h or 38)
        ...
        local function mkPill(row,offset)
                local pill=Instance.new("Frame",row);pill.Size=UDim2.new(0,40,0,21)
                pill.Position=UDim2.new(1,-(offset or 46),0.5,-10.5)
                pill.BackgroundColor3=OFF;pill.BorderSizePixel=0;pill.ZIndex=3
                Instance.new("UICorner",pill).CornerRadius=UDim.new(1,0)
                local dot=Instance.new("Frame",pill);dot.Size=UDim2.new(0,15,0,15);dot.Position=UDim2.new(0,3,0.5,-7.5)
        ...
        local function mkToggle(txt,cb)
                local row=mkRow(38);mkLabel(row,txt)
        ...
        local function mkBox(parent,default,w,xOff,cb)
                ...
                tb.Size=UDim2.new(0,w or 54,0,24);tb.Position=UDim2.new(1,-(xOff or 60),0.5,-12)
                tb.BackgroundColor3=INP;tb.BorderSizePixel=0;tb.Text=tostring(default);tb.TextColor3=W
                tb.Font=Enum.Font.GothamBold;tb.TextSize=12;tb.ClearTextOnFocus=false;tb.ZIndex=5
                Instance.new("UICorner",tb).CornerRadius=UDim.new(0,5)
                local bs=Instance.new("UIStroke",tb);bs.Color=Color3.fromRGB(60,60,60);bs.Thickness=1
        ...
        local function mkKB(parent,kbEntry,cb)
                ...
                btn.Size=UDim2.new(0,50,0,24);btn.Position=UDim2.new(1,-54,0.5,-12)
                ...
                btn.Font=Enum.Font.GothamBold;btn.TextSize=10;btn.ZIndex=5
        ...
        pbStroke.Color=Color3.fromRGB(180,180,185);pbStroke.Thickness=1.3;pbStroke.Transparency=0.35
        local fpsLbl=Instance.new("TextLabel",pbFrame)
        fpsLbl.Size=UDim2.new(0.55,0,0,18)
        fpsLbl.Position=UDim2.new(0,10,0,6)
        fpsLbl.BackgroundTransparency=1;fpsLbl.ZIndex=53
        ...
        progressPct.BackgroundTransparency=1;progressPct.ZIndex=53
        progressPct.Text="0%"
        ...
        progressRadLbl.BackgroundTransparency=1;progressRadLbl.ZIndex=53
        progressRadLbl.Text=string.format("Radius: %.4g",Steal.StealRadius)
        progressRadLbl.TextColor3=Color3.fromRGB(180,180,180)
        progressRadLbl.Font=Enum.Font.GothamMedium;progressRadLbl.TextSize=11
        ...
        pbTrack.BackgroundColor3=Color3.fromRGB(30,30,32);pbTrack.BorderSizePixel=0;pbTrack.ZIndex=51
        Instance.new("UICorner",pbTrack).CornerRadius=UDim.new(1,0)
        local trackStroke=Instance.new("UIStroke",pbTrack)
        trackStroke.Color=Color3.fromRGB(90,90,95);trackStroke.Thickness=1;trackStroke.Transparency=0.5
        progressFill=Instance.new("Frame",pbTrack)
        progressFill.Size=UDim2.new(0,0,1,0);progressFill.BackgroundColor3=Color3.fromRGB(210,210,215)
        progressFill.BorderSizePixel=0;progressFill.ZIndex=52
        ...
        do
                local row=mkRow(38);mkLabel(row,"Mode")
                ...
                modeValLbl.Font=Enum.Font.GothamBlack;modeValLbl.TextSize=12;modeValLbl.TextXAlignment=Enum.TextXAlignment.Right
        ...
        mkSect("Combat")
        do
                local abRow=mkRow(38);mkLabel(abRow,"Auto Bat")
        ...
        do
                local row=mkRow(38);mkLabel(row,"Bat Speed")
        ...
        do
                local row=mkRow(38);mkLabel(row,"Bat Vertical Speed")
        ...
        mkSect("Steal")
        do
                local row=mkRow(38);mkLabel(row,"Radius")
                radInput=mkBox(row,Steal.StealRadius,50,56,function(v)
                        if v>=0.5 and v<=300 then Steal.StealRadius=v end;saveConfig()
                end)
        end
        do
                local row=mkRow(38);mkLabel(row,"Duration (s)")
                mkBox(row,Steal.StealDuration,50,56,function(v)
                        if v and v>0 and v<=5 then Steal.StealDuration=v end;saveConfig()
                end)
        end
        do
                local stealRow=mkRow(38);mkLabel(stealRow,"Auto Steal")
        ...
        mkSectMerge("Misc")
        do
                local row=mkRow(38);mkLabel(row,"Instant Reset")
        ...
        do
                local row=mkRow(38);mkLabel(row,"FOV Value")
        ...
        do
                local row=mkRow(38);mkLabel(row,"Custom Sky")
                ...
                V.skyValLbl.TextColor3=RED;V.skyValLbl.Font=Enum.Font.GothamBlack;V.skyValLbl.TextSize=11
        ...
        do
                local row=mkRow(38);mkLabel(row,"Drop Brainrot")
        ...
        do
                local row=mkRow(38);mkLabel(row,"TP Down")
        ...
        do
                local row=mkRow(38);mkLabel(row,"Auto TP Height")
        ...
        do
                local row=mkRow(38);mkLabel(row,"Save Now")
                local btn=Instance.new("TextButton",row)
                btn.Size=UDim2.new(0,58,0,24);btn.Position=UDim2.new(1,-62,0.5,-12)
                btn.BackgroundColor3=Color3.fromRGB(60,60,60)
                btn.BorderSizePixel=0
                btn.Text="SAVE";btn.TextColor3=Color3.fromRGB(255,255,255)
                btn.Font=Enum.Font.GothamBlack;btn.TextSize=11
                Instance.new("UICorner",btn).CornerRadius=UDim.new(0,5)
                local bs=Instance.new("UIStroke",btn);bs.Color=Color3.fromRGB(255,255,255);bs.Thickness=1;bs.Transparency=0.4
        ...
        local function makeMobileButton(key, label, onToggle)
                ...
                stroke.Color = Color3.fromRGB(50, 52, 60); stroke.Thickness = 1; stroke.Transparency = 0.3
                ...
                btn.Visible = not _G._VezyHideSideBtns
...
loadConfigState()
buildSecretDuelGUI()
print("Secret Duel Loaded - Black & White Theme with Texture ID 6974075369")
print(".gg/secrethub")
